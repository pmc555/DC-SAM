...
class ThreeDMatchPairDataset(torch.utils.data.Dataset):
    ...
    def _compute_overlap_mask(self):
        ...
        for index in tqdm(range(self.__len__())):
            ...
            # get point cloud
            ref_points = torch.load(osp.join(self.data_root, metadata['pcd0']), weights_only=False)
            src_points = torch.load(osp.join(self.data_root, metadata['pcd1']), weights_only=False)
