...
def solve_local_rotations(Am, Bm, weights=None, weight_threshold=0):
    ...
    Output:
        - R, t
    """
    # Guard: VN equivariant features (Am/Bm) can contain NaN/inf when fp16
    # overflows. Return identity rotations as a safe no-op hypothesis rather
    # than crashing the SVD.
    bs = Am.shape[0]
    if not (torch.isfinite(Am).all() and torch.isfinite(Bm).all()):
        return torch.eye(3, device=Am.device).unsqueeze(0).expand(bs, -1, -1).contiguous()
    # Force float32 for all operations: AMP autocast would downcast @ to fp16,
    # and torch.det / torch.svd are not implemented for Half.
    with torch.amp.autocast('cuda', enabled=False):
        ...
        # find rotation — SVD on CPU (avoids fp16 CUDA kernel limitation)
        device = H.device
        U, S, Vt = torch.svd(H.cpu())
        U, S, Vt = U.to(device), S.to(device), Vt.to(device)
        delta_UV = torch.det(Vt @ U.permute(0, 2, 1))
        eye = torch.eye(3)[None, :, :].repeat(bs, 1, 1).to(device)
        eye[:, -1, -1] = delta_UV
...
def weighted_procrustes(
    ...
    with torch.amp.autocast('cuda', enabled=False):
        ...
        device = H.device
        U, _, V = torch.svd(H.cpu())  # H = USV^T, SVD on CPU
        Ut, V = U.transpose(1, 2).to(device), V.to(device)
...
def procrustes(
    ...
    with torch.amp.autocast('cuda', enabled=False):
        ...
        batch_size = src_points.shape[0]
        valid_points = valid_points.float().unsqueeze(2)
        ...
        if src_feats is not None:
            src_points_centered = src_feats.float()
            ref_points_centered = ref_feats.float()
            valid_points = soft_weight(src_feats.float(), ref_feats.float()).unsqueeze(2)
