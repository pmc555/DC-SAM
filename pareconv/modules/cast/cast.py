...
from pareconv.modules.ops.pairwise_distance import pairwise_distance1
...
from pareconv.modules.cast.linear_transformer import LinearTransformerLayer

from pareconv.modules.cast.spot_attention import SparseTransformerLayer, Upsampling, Downsampling
...
class SpotGuidedTransformer(nn.Module):
    ...
    def forward(self, ref_points, src_points, ref_feats, src_feats, ref_points_c, src_points_c, ref_feats_c,
                src_feats_c):
        """
        Args:
        ...
        with torch.no_grad():
            ref_dists = pairwise_distance1(ref_points, ref_points)  # (B, N, N)
            src_dists = pairwise_distance1(src_points, src_points)  # (B, M, M)
...
class SpotGuidedGeoTransformer(nn.Module):
    def __init__(
        ...
        self.k = k  # num of neighbor points whose corresponding patches are candidate spots
        self.spots = spots  # num of neighbor points whose corresponding patches are selected as spots
        self.down_k = down_k  # num of nodes for down sampling and fusion with semi-dense points
        self.spot_k = spot_k  # num of points in a spot
        self.blocks = blocks
        ...
        self.in_proj1 = nn.Linear(input_dim_c, hidden_dim)
        self.in_proj2 = nn.Linear(input_dim_f, hidden_dim)
        self.linear_cross_attention = LinearTransformerLayer(
            hidden_dim, num_heads, dropout, activation_fn
        )

        self.embed = GeometricStructureEmbedding(hidden_dim, sigma_d, sigma_a, angle_k, reduction_a)
        ...
        self.spot_guided_attentions = nn.ModuleList()
        self.consistency_aware_attentions = nn.ModuleList()
        self.out_proj_c = nn.Linear(hidden_dim, output_dim_c)
        self.out_proj_f = nn.Linear(hidden_dim, output_dim_f)

        for _ in range(self.blocks):
            self.upsampling.append(Upsampling(hidden_dim, hidden_dim))
            self.downsampling.append(Downsampling(hidden_dim, hidden_dim))
    ...
    def forward(self, ref_points, src_points, ref_feats, src_feats, ref_points_c, src_points_c, ref_feats_c,
        ...
        for i in range(self.blocks):
            ...
            new_ref_feats_c, _ = self.cross_attentions[i](ref_feats_c, src_feats_c)
            new_src_feats_c, _ = self.cross_attentions[i](src_feats_c, ref_feats_c)
            # print(f" {i}new_ref_feats_c: {new_ref_feats_c.shape}")
            # print(
            #     f"[Upsample {i}] latent: {new_ref_feats.shape}, query: {new_ref_feats_c.shape}, idx: {ref_idx_up.shape}")
            ...
            new_ref_feats = self.consistency_aware_attentions[i](
                ref_feats, src_feats, src_token_indices.unsqueeze(1)
            )
            new_src_feats = self.consistency_aware_attentions[i](
                src_feats, ref_feats, ref_token_indices.unsqueeze(1)
            )
            ...
            # ref_feats_c = self.downsampling[i](new_ref_feats_c, new_ref_feats, ref_points_c, ref_xyz_down, ref_idx_down)
            # src_feats_c = self.downsampling[i](new_src_feats_c, new_src_feats, src_points_c, src_xyz_down, src_idx_down)
            # print(f"[downsampling {i}]ref_feats_c: {ref_feats_c.shape}")

        ref_compatibility = torch.stack(ref_compatibility, dim=-1)
        src_compatibility = torch.stack(src_compatibility, dim=-1)
        ref_feats_c = self.out_proj_c(new_ref_feats_c)
        src_feats_c = self.out_proj_c(new_src_feats_c)

        ref_feats_f = self.out_proj_f(new_ref_feats)
        src_feats_f = self.out_proj_f(new_src_feats)
        return ref_feats_c, src_feats_c, ref_feats_f, src_feats_f, correlation, ref_compatibility, src_compatibility
