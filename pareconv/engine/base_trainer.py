...
class BaseTrainer(abc.ABC):
    ...
    def optimizer_step(self, iteration):
        if iteration % self.grad_acc_steps == 0:
            # AMP: scaler.step 在梯度含 inf/nan 时自动跳过本次更新
    ...
    def check_invalid_gradients(self):
        for name, param in self.model.named_parameters():
            ...
            if torch.isnan(param.grad).any():
                self.logger.error(f'NaN in gradient of "{name}"; skipping optimizer step.')
                return False
            if torch.isinf(param.grad).any():
                self.logger.error(f'Inf in gradient of "{name}"; skipping optimizer step.')
                return False
        return True
