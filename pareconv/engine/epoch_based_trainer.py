...
class EpochBasedTrainer(BaseTrainer):
    ...
    def train_epoch(self):
        ...
        for iteration, data_dict in enumerate(self.train_loader):
            ...
            # backward & optimization（AMP: 先 scale 再 backward）
    ...
    def inference_epoch(self):
        ...
        # 保存供 run() 的最优模型追踪使用
        self.val_summary_dict = summary_dict
        self.set_train_mode()
    ...
    def run(self):
        ...
        if self.args.resume:
            snapshot_path = self.args.snapshot if self.args.snapshot is not None else osp.join(self.snapshot_dir, 'snapshot.pth.tar')
            self.load_snapshot(snapshot_path) #这两行原代码是self.load_snapshot(osp.join(self.snapshot_dir, 'snapshot.pth.tar'))
        ...
        while self.epoch < self.max_epoch:
            ...
            self.inference_epoch()
            # 最优模型追踪（以验证集 RR 为基准）
            val_rr = float(self.val_summary_dict.get('RR', -1.0))
            if val_rr > self.best_val_metric:
                self.best_val_metric = val_rr
                self.best_epoch = self.epoch
                self.save_snapshot('best_model.pth.tar')
                self.logger.info(
                    f'[Best] epoch {self.epoch}: RR={val_rr:.4f} → saved best_model.pth.tar'
                )
        if not self.debug:
            wandb.finish()
