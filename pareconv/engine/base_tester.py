...
def inject_default_parser(parser=None):
    ...
    # 只在 --snapshot 尚未注册时才添加（允许各 test.py 自定义默认路径）
    if not any('--snapshot' in getattr(a, 'option_strings', []) for a in parser._actions):
...
class BaseTester(abc.ABC):
    ...
    def load_snapshot(self, snapshot):
        ...
        if missing:
            self.logger.warning('Missing keys (new params, will use random init): {}'.format(missing))
        if unexpected:
            self.logger.warning('Unexpected keys (ignored from checkpoint): {}'.format(unexpected))
