...
# data
_C.data = edict()
_C.data.dataset_root = 'F:/dataset/PUBLIC_3D/KITTI'
_C.data.metadata_root = osp.join(_C.root_dir, 'data', 'KITTI', 'metadata')
# train data
_C.train = edict()
...
_C.backbone.subsample_ratio = 2.5
...
_C.backbone.output_dim = 64
_C.backbone.use_v1 = False  # False=原版(兼容pretrained); True=backboneV1(ChannelAggregationFFN, 从头训练)
...
_C.geotransformer.blocks = blocks = ['self', 'cross', 'self', 'cross', 'self', 'cross'] #['self', 'cross', 'caa-sga', 'cross', 'self', 'cross']
#_C.geotransformer.blocks1 = blocks1 = ['caa-sga', 'caa-sga']
...
_C.fine_matching.num_hypotheses = 1500
...
# loss - Fine level
_C.fine_loss = edict()
_C.fine_loss.positive_radius = 0.6   # 课程学习的终点值
_C.fine_loss.negative_radius = 1.2   # 课程学习的终点值
_C.fine_loss.positive_margin = 0.1
_C.fine_loss.negative_margin = 1.4
_C.fine_loss.curriculum = True  # True=启用匹配半径课程退火 (2×init→init)
