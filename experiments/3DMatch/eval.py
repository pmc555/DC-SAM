...
try:
    from pareconv.utils.open3d import registration_with_ransac_from_correspondences, registration_with_ransac_from_feats
except ImportError:
    registration_with_ransac_from_correspondences = None  # open3d not available (Python 3.13); only affects --method ransac
...
def make_parser():
    ...
    parser.add_argument('--benchmark', default='3DMatch', choices=['3DMatch', '3DLoMatch'],  help='test benchmark')
    parser.add_argument('--method', default='fhp', choices=['fhp', 'ransac', 'svd'], help='registration method')
...
def eval_one_epoch(args, cfg, logger):
    ...
    for scene_root in scene_roots:
        ...
        gt_root = osp.join(cfg.data.metadata_root, 'benchmarks', benchmark, scene_name)
        ...
        static_errors[scene_name] = OrderedDict()
